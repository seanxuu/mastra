...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'boolean',
    description:
      'When true, violations (soft and hard) include a per-provider/model cost breakdown queried via getMetricBreakdown. The breakdown query runs only when a violation trips, never on the happy path. If the configured store does not support breakdown queries or the query fails, the violation fires without the breakdown field.',
    isOptional: true,
    defaultValue: 'false',
  },
]}
...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Callback invoked when a cost violation is detected, regardless of strategy. For the warn strategy and for soft thresholds, it fires at most once per request per threshold level. The violation detail includes a threshold field ("soft" or "hard") and, when includeBreakdown is enabled, a breakdown array. Use for side effects like alerting, logging to external systems, or emailing users. Errors thrown by this callback are caught and logged through the Mastra logger.',
    isOptional: true,
  },
  {
