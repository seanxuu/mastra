...
description: "API reference for TokenCostControl, a processor that enforces monetary cost limits on agent runs."
packages:
  - "@mastra/core"
---
...
> **Renamed from `CostGuardProcessor`.** The `CostGuardProcessor` export (and its `CostGuard*` option and detail types) remains available as a deprecated alias for the same class, including the `'token-cost-control'` processor id. The intermediate `CostControlProcessor` name is also available as a deprecated alias. Migrate imports to `TokenCostControl`.
