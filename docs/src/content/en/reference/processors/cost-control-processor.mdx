---
title: "Reference: CostControlProcessor | Processors"
description: "API reference for CostControlProcessor, a processor that enforces monetary cost limits on agent runs."
packages:
  - "@mastra/core"
---
...
The `CostControlProcessor` enforces monetary cost limits across the agentic loop, blocking or warning when a configurable cost threshold is exceeded.
...
> **Renamed from `CostGuardProcessor`.** The `CostGuardProcessor` export (and its `CostGuard*` option and detail types) remains available as a deprecated alias for the same class, including the `'cost-control'` processor id. Migrate imports to `CostControlProcessor`.
...
> **Agent attribution only.** Cost is attributed via the `entityType: 'agent'` metric filter. Model calls made outside an agent run — for example, direct model usage in workflow steps — have no agent parent span and are not counted by this guard.
...
```typescript
import { CostControlProcessor } from '@mastra/core/processors'
...
costControl.onViolation = ({ detail }) => {
  console.log(
...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Callback invoked when a cost violation is detected, regardless of strategy. For the warn strategy and for soft thresholds, the guard calls it with a CostControlViolationDetail (usage, limit, threshold, and optional breakdown) at most once per request per threshold level. For the block strategy, the processor runner invokes it with the TripWire metadata as the detail (see Error behavior). Use for side effects like alerting, logging to external systems, or emailing users. Errors thrown by this callback are caught and logged through the Mastra logger.',
    isOptional: true,
  },
  {
...
When the `block` strategy is active (default), `CostControlProcessor` calls `abort()` with `retry: false` when the cost limit is exceeded. The TripWire metadata includes:
...
- `threshold`: Always `'hard'` — only the hard limit aborts
