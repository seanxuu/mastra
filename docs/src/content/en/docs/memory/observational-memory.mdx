...
```typescript title="src/mastra/memory.ts"
const subconscious = new Subconscious({
  pins: { maxCharacters: 2000 },
})
```

When enabled, the curator maintains a reserved `pinned-knowledge` page in its default scope and its body is delivered on every turn as a snapshot state signal. Unchanged pins reuse their cache key, so a stable pin set costs nothing after the first turn. `maxCharacters` defaults to 2000 and is enforced on the write path, so the curator can't grow the page past its budget. Pins are off by default because their cost is per turn and permanent.
