...
Subconscious memory extends OM with durable, scoped knowledge. It captures entities and facts during observation, surfaces relevant reminders, and maintains curated pages and reusable skills during reflection.

Subconscious memory requires a storage adapter with knowledge support, an embedder, and a vector store. Semantic retrieval is required and doesn't fall back to lexical-only search.
...
export const memory = new Memory({
  ...
  embedder: openai.embedding('__AI_SDK_OPENAI_EMBEDDING_MODEL__'),
  options: {
    observationalMemory: {
...
The default configuration enables four built-in agents:

- `capture`: Extracts entities and facts in the Observer's structured call.
- `remind`: Searches scoped knowledge after observation and sends at most one grouped `<remembered>` signal.
- `curate`: Merges duplicate entities, repairs facts, and writes pages after reflection.
- `learn`: Distills repeated procedures into scoped skill records after reflection.

Use `observation` and `reflection` to select built-ins. Capture accepts custom instructions or a custom schema and hook, but always shares the Observer's structured call and model. The agent-backed remind, curate, and learn built-ins accept instruction, model, and `maxSteps` overrides. Set `tools: false` to remove the `knowledge_search`, `knowledge_read`, and `knowledge_browse` tools from the main agent.

```typescript title="src/mastra/memory.ts"
const subconscious = new Subconscious({
  observation: ['capture', { name: 'remind', maxSteps: 3 }],
  reflection: ['curate', 'learn'],
...
Knowledge uses the scope chain `org → resource → thread`. Reads include the current scope and visible ancestors but exclude sibling-thread data. `maxScope` is a mechanical ceiling: neither agents nor tools can promote a fact above it.

Entities provide identity. Facts contain scoped prose, provenance, capture time, and an optional event time. Pages are curated documents with wikilinks; they aren't graph entities. `knowledge_search` combines lexical and semantic ranking, while `knowledge_read` and `knowledge_browse` retrieve records, mentions, and backlinks within the caller's scope.

Curator removal soft-deletes facts. Normal reads and search exclude deleted facts. Restoration is available only through the application storage API, not through agent tools.

### Pinned knowledge

Reminders are reactive: they surface knowledge only when the remind agent judges it relevant to the current observations. Some knowledge should apply without being asked for, such as standing instructions and hard constraints. Set `pins` to enable a small always-present tier.

```typescript title="src/mastra/memory.ts"
const subconscious = new Subconscious({
  pins: { maxPins: 20, maxCharacters: 2000 },
})
```

A pin is a knowledge fact on a reserved entity, so each pin has a stable id, its own scope, and provenance. The curator maintains the set with `knowledge_pin`, `knowledge_edit_pin`, and `knowledge_unpin`. Unpinning soft-deletes the fact; editing replaces it under a new id, since facts are append-only.

Delivery is a state signal projected by a processor. When no snapshot is in the visible context (first emission, or observational memory evicted it) the full set is emitted as a `<pinned-knowledge>` snapshot; a change while a snapshot is visible emits a `<pinned-knowledge-update>` delta carrying only that change; an unchanged turn emits nothing, so a stable pin set costs nothing after the first turn. Unpinning the last pin emits an empty snapshot so the lane clears rather than replaying stale content.

`maxPins` (default 20) and `maxCharacters` (default 2000, total across the set) are enforced in the pin tools, so the set cannot grow past its budget. Pins are off by default because their cost is per turn and permanent.

### Signals, costs, and failures

Activity uses a bounded state signal with cache-key deduplication. Unchanged snapshots don't add another signal. Set `activity: false` to suppress routine snapshots; failure states still emit so clients can surface background errors. Reminders use the reactive signal lane and remain quiet when no relevant knowledge is found.

Capture shares the Observer's structured call and model. Remind adds at most one bounded agent run per observation cycle. Curate and learn run sequentially after a committed reflection and use separate durable cursors. The agent-backed built-ins enforce `maxSteps`; all built-ins propagate cancellation and can't recursively enable Subconscious memory.

Background failures don't roll back observation or reflection. Mastra adds the failure to the activity state's `errors` array and emits a `data-subconscious-error` part for clients that display runtime status.

:::note
Subconscious memory depends on the current request context for organization, resource, and thread scope. Knowledge tools fail closed when required scope identifiers are missing.
:::
